...
function tar.untar(filename, destdir)
	...
	---@type fun(path: Path | string): boolean, string?
	local make_dir = utilities.memoize(function(path) return Path.new(path):create_directory(true) end)
	while true do
		local block
		repeat
		...
		local pathname = Path.new(destdir, header.name)
		...
		elseif header.typeflag == "file" then
			local dirname = pathname:parent_directory()
			if dirname ~= "" then
			...
			local file_handle
			file_handle, err = io.open(tostring(pathname), "wb")
			...
			file_handle:close()
			pathname:set_time(header.mtime)
