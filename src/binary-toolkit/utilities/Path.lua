...
function Path.new(path, ...)
    if type(path) == "table" then return setmetatable(path, Path) end
    --[[@cast path string]]
    local parts = {}
...
---@param access_time integer | osdateparam
---@param modifcation_time (integer | osdateparam)?
---@return boolean, string?
function Path:set_time(access_time, modifcation_time)
    access_time = type(access_time) == "table" and os.time(access_time) or access_time
    modifcation_time = modifcation_time and (type(modifcation_time) == "table" and os.time(modifcation_time) or modifcation_time) or access_time

    --[[@cast access_time integer]]
    --[[@cast modifcation_time integer]]
    if self:type() == "directory" then
        for entry in self:entries() do
            local ok, err = entry:set_time(access_time, modifcation_time)
            if not ok then return false, err end
        end

        return true
    else
        return lfs.touch(tostring(self), access_time, modifcation_time)
    end
end
